...
router.get('/', async (req, res, next) => {
    ...
    try {
      ...
      data = response.data;
      console.log("Receipes response from API: " + JSON.stringify(data.recipes) + '\n');
    }catch(error) {
        console.error(error);
    };
    ...
    var recipe = data.recipes.map(item => ({
        ...
        weightWatcherPoints: item.weightWatcherSmartPoints,
        sourceUrl: item.sourceUrl,
        spoonacularSourceUrl: item.spoonacularSourceUrl,
        imgUrl: item.image,
        creditText: item.creditsText,
    ...
    console.log("Recipe Map: " + JSON.stringify(recipe) + '\n');
    res.json(recipe);
});
